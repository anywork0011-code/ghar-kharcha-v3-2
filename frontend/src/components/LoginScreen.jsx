...
export default function LoginScreen({ onLogin, showToast }) {
  ...
  const inp = { width:'100%',background:'var(--card)',border:'1px solid var(--border)',borderRadius:12,padding:'13px 16px',color:'var(--text)',fontSize:15 }
    ...
    <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'0 22px',position:'relative',overflow:'hidden',background:'#0f0f13'}}>
      ...
      <div style={{width:'100%',animation:'fadeUp .4s ease'}}>
        <div style={{textAlign:'center',marginBottom:32}}>
          ...
          <p style={{fontSize:12,color:'var(--muted)',marginTop:5}}>तुमचे घरगुती खर्च व्यवस्थापन</p>
        </div>

        <div style={{background:'var(--card)',borderRadius:20,padding:'24px 20px',border:'1px solid var(--border)',boxShadow:'0 16px 48px rgba(0,0,0,.4)'}}>
          <p style={{fontSize:11,color:'var(--text2)',fontWeight:700,letterSpacing:1.2,marginBottom:16}}>🔐 प्रवेश करा</p>
          <div style={{marginBottom:12}}>
            <label style={{fontSize:11,color:'var(--text2)',fontWeight:700,display:'block',marginBottom:6}}>वापरकर्ता नाव</label>
          ...
          <div style={{marginBottom:20}}>
            <label style={{fontSize:11,color:'var(--text2)',fontWeight:700,display:'block',marginBottom:6}}>पासवर्ड</label>
            <div style={{position:'relative'}}>
              ...
              <button onClick={()=>setShowPw(!showPw)} style={{position:'absolute',right:12,top:'50%',transform:'translateY(-50%)',background:'transparent',color:'var(--muted)',fontSize:16}}>{showPw?'🙈':'👁️'}</button>
            </div>
          </div>
          ...
          {/* <div style={{marginTop:14,padding:'10px 12px',background:'#0f0f13',borderRadius:9,border:'1px solid var(--border)'}}>
            <p style={{fontSize:11,color:'var(--muted)',fontWeight:700,marginBottom:4}}>🔑 Default Admin:</p>
