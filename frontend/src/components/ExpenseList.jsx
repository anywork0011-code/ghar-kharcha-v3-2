...
export default function ExpenseList({ expenses, filters, setFilters, onDelete, onEdit, onStatusChange, adminView, loading }) {
  ...
  return (
    <div style={{paddingBottom:20}} className="fade">
      <div style={{background:'#16161e',padding:'12px 14px',borderBottom:'1px solid #2a2a40',position:'sticky',top:0,zIndex:50}}>
        ...
        <input value={filters.search||''} onChange={e=>set('search',e.target.value)} placeholder="🔍 शोधा..."
          style={{width:'100%',background:'#1a1a25',border:'1px solid #2a2a40',borderRadius:9,padding:'9px 12px',color:'#f1f0ff',fontSize:12}}/>
        {showFilt && (
          <div style={{marginTop:10,display:'flex',flexDirection:'column',gap:9}}>
            <div>
              <p style={{fontSize:10,color:'#6b6b88',fontWeight:700,marginBottom:5}}>कालावधी</p>
            ...
            {filters.filter==='custom' && (
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                {[['startDate','पासून',showCalS,setShowCalS],['endDate','पर्यंत',showCalE,setShowCalE]].map(([k,l,show,setShow])=>(
                  <div key={k}>
                    <p style={{fontSize:10,color:'#6b6b88',fontWeight:700,marginBottom:4}}>{l}</p>
                    <button onClick={()=>setShow(true)} style={{width:'100%',background:'#1a1a25',border:'1px solid #2a2a40',borderRadius:8,padding:'8px 10px',color:filters[k]?'#f1f0ff':'#6b6b88',fontSize:11,textAlign:'left',display:'flex',justifyContent:'space-between'}}>
            ...
            <div>
              <p style={{fontSize:10,color:'#6b6b88',fontWeight:700,marginBottom:5}}>स्थिती</p>
              <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
            ...
            <div>
              <p style={{fontSize:10,color:'#6b6b88',fontWeight:700,marginBottom:5}}>क्रमवारी</p>
              <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
      ...
      {expenses.length>0 && (
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',background:'#1a1a25',borderBottom:'1px solid #2a2a40',padding:'8px 14px'}}>
          {[{l:'भरावे',v:fmt(tPay),c:'#ef4444'},{l:'मिळणार',v:fmt(tRec),c:'#22c55e'},{l:'शिल्लक',v:(tRec-tPay>=0?'+':'')+fmt(Math.abs(tRec-tPay)),c:tRec-tPay>=0?'#22c55e':'#ef4444'}].map((s,i)=>(
            <div key={s.l} style={{textAlign:'center',borderRight:i<2?'1px solid #2a2a40':'none'}}>
              <div style={{fontSize:9,color:'#6b6b88',fontWeight:700}}>{s.l}</div>
      ...
      <div style={{padding:'10px 13px',display:'flex',flexDirection:'column',gap:7}}>
        {loading ? (
          ...
          <div style={{textAlign:'center',padding:'50px 20px'}}>
            ...
            <p style={{color:'#6b6b88'}}>कोणत्या नोंदी सापडल्या नाहीत</p>
          </div>
        ) : expenses.map(e=>(
