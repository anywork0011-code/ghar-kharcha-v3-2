...
export default function Dashboard({ todayExpenses, summary, yearlyData, deadlines, onDelete, onEdit, onStatusChange, userName, userWhatsapp }) {
  ...
  return (
    <div style={{paddingBottom:20}}>
      {/* Hero */}
      <div style={{background:'linear-gradient(160deg,#1e1e2a 0%,#16161e 100%)',padding:'16px 16px 20px',borderBottom:'1px solid var(--border)',position:'relative',overflow:'hidden'}}>
        ...
        <p style={{fontSize:11,color:'var(--text2)',fontWeight:700,letterSpacing:1.4}}>👋 नमस्कार, {userName}</p>
        ...
        <div style={{marginTop:10,background:balance>=0?'rgba(34,197,94,.06)':'rgba(239,68,68,.06)',borderRadius:12,padding:'9px 14px',border:`1px solid ${balance>=0?'rgba(34,197,94,.15)':'rgba(239,68,68,.15)'}`,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          <p style={{fontSize:11,color:'var(--text2)',fontWeight:600}}>निव्वळ शिल्लक</p>
      ...
      {yearlyData?.length>0 && (
        <div style={{padding:'12px 14px 0'}}>
          <p style={{fontSize:10,color:'var(--muted)',fontWeight:700,letterSpacing:1.4,textTransform:'uppercase',marginBottom:9}}>📊 गेल्या 5 वर्षांचा सारांश</p>
          <div style={{display:'flex',flexDirection:'column',gap:7}}>
            {yearlyData.map(y=>(
              <div key={y.year} onClick={()=>setExpandYr(expandYr===y.year?null:y.year)}
                style={{background:'var(--card)',borderRadius:13,border:'1px solid var(--border)',overflow:'hidden',cursor:'pointer'}}>
                <div style={{padding:'10px 13px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                  <div style={{display:'flex',alignItems:'center',gap:9}}>
                    ...
                    <span style={{fontSize:9,color:'var(--muted)',background:'var(--card2)',padding:'2px 7px',borderRadius:20}}>{y.count} नोंदी</span>
                  </div>
                  <div style={{display:'flex',alignItems:'center',gap:8}}>
                    ...
                    <span style={{fontSize:11,color:'var(--muted)'}}>{expandYr===y.year?'▲':'▼'}</span>
                  </div>
                </div>
                {expandYr===y.year && (
                  <div style={{padding:'9px 13px 11px',borderTop:'1px solid var(--border)',display:'grid',gridTemplateColumns:'1fr 1fr',gap:7}}>
                    <div style={{background:'rgba(239,68,68,.06)',borderRadius:9,padding:'8px 10px',border:'1px solid rgba(239,68,68,.15)'}}>
                      <p style={{fontSize:9,color:'var(--text2)',marginBottom:2}}>भरावे लागेल</p>
                    ...
                    <div style={{background:'rgba(34,197,94,.06)',borderRadius:9,padding:'8px 10px',border:'1px solid rgba(34,197,94,.15)'}}>
                      <p style={{fontSize:9,color:'var(--text2)',marginBottom:2}}>मिळणार</p>
                ...
                <div key={e._id} style={{background:'rgba(239,68,68,.06)',borderRadius:12,border:`1px solid ${diff<=0?'rgba(239,68,68,.3)':'rgba(245,158,11,.3)'}`,padding:'11px 13px'}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                    <div style={{flex:1,minWidth:0}}>
                      <p style={{fontWeight:700,fontSize:13,color:'var(--text)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{e.name}</p>
      ...
      <div style={{padding:'12px 14px 0'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:9}}>
          <p style={{fontSize:10,color:'var(--muted)',fontWeight:700,letterSpacing:1.4,textTransform:'uppercase'}}>📝 आजच्या नोंदी</p>
          <span style={{fontSize:10,color:'var(--muted)',background:'#1e1e2a',padding:'2px 9px',borderRadius:20,border:'1px solid var(--border)'}}>{todayExpenses.length}/30</span>
        </div>
        {todayExpenses.length===0 ? (
          <div style={{textAlign:'center',padding:'32px 16px',background:'var(--card)',borderRadius:14,border:'1px dashed #2a2a40'}}>
            <div style={{fontSize:34,marginBottom:10}}>🌟</div>
            <p style={{color:'var(--muted)',fontSize:13}}>आज कोणत्या नोंदी नाहीत</p>
